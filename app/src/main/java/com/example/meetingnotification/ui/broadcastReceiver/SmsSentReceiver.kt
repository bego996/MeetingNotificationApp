...
class SmsSentReceiver: BroadcastReceiver() {   // Eine Klasse, die `BroadcastReceiver` erweitert und eine `SmsSendingService`-Instanz empfängt

    // Überschreibt die `onReceive`-Methode, um auf Broadcasts zu reagieren
    override fun onReceive(context: Context, intent: Intent) {
        ...
        FirebaseCrashlytics.getInstance().log("smsReceiverCalled() with ResultCode: $resultCode")
        ...
        if (resultCode == Activity.RESULT_OK || resultCode == 4 || resultCode == 1) {     // Überprüft, ob die SMS erfolgreich gesendet wurde, indem das `resultCode` mit `RESULT_OK` verglichen wird. 4 war für ein anderes Device als OK.
            ...
            Log.d(TAG,"serviceState = $service")
            FirebaseCrashlytics.getInstance().log("serviceState = $service")
            ...
            FirebaseCrashlytics.getInstance().log("Size of the Message Queue: $messageQueueSize.")
            ...
            service?.getUpcomingEventForContact(contactId) { event ->
                event.let {
                    ...
                    Log.d(TAG, "Upcoming Event $updatedEvent  set to notified = true")

                    service.sendNextMessage(context)
                    Log.d(TAG, "NextMessageSend()")
                    FirebaseCrashlytics.getInstance().log("NextMessageSend()")
                }
            }
        }
    }
